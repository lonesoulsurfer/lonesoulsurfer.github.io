...
- Now - you need to add a small M2 spacer (around 7mm) to each of the screws.
- You can now add the low profile female header pins to the male ones on the TFT screen and do a test fit.
- If everything fits as it should, you can then solder the female pins on the low profile header to the PCB.
...
- Acid Drip uses four libraries. Install them all via the Library Manager (Sketch → Include Library → Manage Libraries…).
- Search for and install each of these:
- Mozzi Search "Mozzi" — install version 2.0 or late
- Mozzi note: When prompted about dependencies, click Install All.
...
- BeatMachine2.ino
- beats.h
- sample0.h through sample7.h (all 8 sample files)
...
- Go to Tools and configure these settings:
- Board - Raspberry Pi Pico / Waveshare RP2040 Zero
- CPU Speed - 133 MHz (default) or 200 MHz
...
- Compile the sketch (this takes 30–60 seconds the first time)
- Upload the binary to the RP2040
- The board will reboot automatically and start running
...
- The TFT display should light up and show the Acid Drip UI
- The step grid and pot bars should be visible on screen
- Pressing pads should toggle steps on and off
- Pressing pads 1+2 together starts the sequencer
Troubleshooting
...
- I decided to have my synth on an angle facing me so it is easier to play. to do this I cut the 2 side pieces at an angle
- Then measure the front and back pieces and nail and glue them all together
- You will need to sand down the tops of the back and top sections of the case so they follw the angle of the wood if you are also making a case that faces you like I did. I just used my belt sander to do this which worked fine.
- Place the front panel into the case and make sure it all fits well. if the sides aren't flush with the panel, then just sand them a bit until they are
- For the base I used some translucent acylic (it is what I had at hand!) which I think works well in a project like this - you can show off the insides and also see the charging LED.
- Drill some holes and secure it with some screws. You'll need to add some little sticky rubber feet on the bottom as well to stop it from sliding about when playing it
- make a slot in the back of the case to fit the USB C connector for charging and secure it into place
- Now give it a coat of stain or varnish or whatever you want and once dry secure the front panel to the case
...
- KEY — set the root note (C, D, Eb, F, G, Ab, Bb, B)
- RIFF — load one of 8 preset riff patterns to start a bassline (DFLT, SQNCE, FUNK, MINI, JUMP, RAVE, SYNC, DARK)
- SOUND — choose from 8 different synth voices
- WALK — sets an automatic note walk so the pattern evolves over time (up, down, bounce, random, etc.)
- FX — assign a per-step effect such as octave up/down, retrigger, arpeggio, overdrive or bit crush
- TEMPO — pick a preset BPM or tap the pad repeatedly to set tempo by ear
- PLEN — set the pattern length from 1 to 16 steps
- PAT> — controls playback order (forward, reverse, ping-pong, random, etc.)
Accent Edit
...
- CUT — sets how much extra filter brightness an accented step gets
- RES — sets how much extra resonance an accented step gets
- DCY — sets how long the accent's filter sweep takes to decay back to normal
...
- PTCH — global pitch, transposes all drums together on top of any per-drum pitch settings
- DRIV — drive and saturation, pushing the drum bus through a hard-knee saturator for grit; the top end of the knob also adds bit crush for lo-fi crunch
- FILT — bipolar DJ filter; centre is bypass, turning left sweeps a lowpass down from bright to deep, turning right sweeps a highpass up for that classic DJ sweep effect
- CHNC — chance; centre plays the pattern as-is, turning left progressively thins hits (the kick and snare stay anchored longest), turning right adds soft ghost hits on empty steps so the groove fills in organically
- HMNZ — humanize, adding random velocity spread and per-hit micro-timing jitter so the kit drifts off the grid independently and feels less robotic
- SWNG — swing, delaying every other 16th step to push the groove from straight to swung
- ACNT — accent depth; centre is flat, turning right puts emphasis on the downbeats, turning left flips it for an off-beat pushed feel
- FILL — procedural fill engine; choose how often fills happen (every 1, 2, or 4 bars) and what type (HATS, CLAP, SNARE, or KIT). The machine takes over the last few steps before the bar boundary with a building rush, then lands on beat 1 with a kick and open hat. See the Fill Engine section below.
The Fill Engine
